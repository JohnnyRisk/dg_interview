...
def fill_feed_dict(data, labels, x, y_):
    batch_data = tf.convert_to_tensor(data.permute(0, 2, 3, 1).numpy(), dtype=tf.float32)
    batch_labels = tf.convert_to_tensor(labels.numpy(), dtype=tf.float32)
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for phase in ['train','val']:
                for batch, data in enumerate(dataloaders[phase]):
                    ...
                    feed_dict = fill_feed_dict(inputs, labels,
                                               images_placeholder, labels_placeholder,
                                               step)
                    if phase == 'train':
                        _, l, lr, acc = sess.run([optimizer, loss, learning_rate, accuracy],
                                                 feed_dict=feed_dict)
                        ...
                        valid_feed_dict = fill_feed_dict(valid_data,
                                                         valid_labels,
                                                         images_placeholder,
                                                         labels_placeholder,
                                                         step)

                        valid_summary, _, l, lr, valid_acc = sess.run([merged, loss, accuracy],
